...
until software rendering is on. `.cursor/environment.json` writes bun onto
`PATH` and sets `MUNKEL_ELECTRON_DISABLE_GPU=1`, `ELECTRON_DISABLE_SANDBOX=1`,
and `LIBGL_ALWAYS_SOFTWARE=1` in `~/.profile` / `~/.bashrc` so later `bun run
